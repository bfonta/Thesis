...
In this Section, we list the most prominent challenges a successful future theory should ideally address [[cite:&griffiths;&thomson]].
...
+ *Dark Matter*:
  ...
  Alternative explanations for the observed discrepancies nevertheless exist, most notably \ac{MOND} theories [[cite:&mond]], but are generally disfavored by the scientific community.
...
+ *\Ac{CP} violation and baryogenesis*:
  ...
  In the \ac{SM}, the violation of \ac{CP} symmetry can only be accommodated in the parameters of the \ac{CKM} and \ac{PMNS} matrices.
...
+ *The nature of neutrinos*:
  ...
  The most striking evidence for Majorana-type neutrinos would be the observation of neutrino-less double $\beta\text{-decay}$ $(\text{Z},\text{A}) \rightarrow (\text{Z}+2,\text{A}) + 2e^{-}$, with Z the atomic number and A the mass number, which cannot occur if the neutrinos are of Dirac-type.
