...
Modifications of the Higgs self-coupling are parameterized with the $\kappa\text{-framework}$ presented in [[#sec:nonres_BSM_hh]], which is known to be equivalent to an \ac{EFT} approach with the introduction of higher-dimensional operators only affecting $\lh{3}$.
