...
This configuration was chosen because it allows the convenient definition of uniform groups of neighbouring cells to form \acp{TP}, as shown in [[fig:tcs_geometry]].
...
This combination is preferred over the treshold algorithm due to the fixed output data size, which leads to a simpler, bufferless \ac{BE} data unpacking.
...
+ *Histogramming*:
  ...
  Energy deposits of neutral particles originating from the centre of the detector and spanning several layers will thus lie in a single \rz{} bin.
...
+ *Clustering*:
  ...
  The second algorithm, called =max_energy=, prioritizes an association based on seed energy, where the highest energy seed is associated to all \acp{TC} within its matching radius, the second-highest energy seed is associated to the remaining \acp{TC} within its (different) matching radius, and so forth.
...
We refer to "interpretations" since at the \ac{TPG} no particle identification is performed.
