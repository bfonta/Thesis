...
The response is here defined as the difference between clustered and generated energy divided by generated energy, where zero corresponds to reconstructing all generated energy.
...
We also test a seeding window of size \num{2}, but find no significant difference relative to the default size \num{1}.
