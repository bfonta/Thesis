...
Despite \num{200} \ac{PU} validation checks samples suggesting a correct data processing behaviour, extensive studies are necessary.
...
It is currently not clear whether seeding algorithms require information from the \ac{Sci} section.
...
During the testing of the hexagonal seeding, for instance, it was found that certain algorithmic steps performed in a determined order with specific parameters lead to failures.
