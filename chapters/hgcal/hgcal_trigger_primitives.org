...
The \ac{TPG} must fit within a latency of \SI{\sim 5}{\micro\second}, or \num{200} \acp{BX}, taken from the total \SI{12.5}{\micro\second} latency allocated to \ac{L1} [[cite:&l1TDR]], and is constrained by the available bandwidth between the \ac{FE} and the \ac{BE}.
The bandwidth is limited by several factors, including total power consumption, the multiplicity of routing lines, the space available for electronic boards in the \ac{USC}, and the overall cost of hardware, among which expensive \acp{FPGA} and optical links [[cite:&jb_hdr]].
...
\ac{HGCAL} is the second subdetector in \ac{CMS} with the most allocated resources at \ac{L1}, due to its high granularity.
...
The \ac{BE} have been implemented on the \ac{ATCA} infrastructure [[cite:&atca_website]].
...
Being the entry point to the \ac{FE} electronics, the DAQ is also with synchronous fast control, asynchronous slow control, and transporting the timing information from \ac{CMS} to the \ac{FE}.
...
Additionally, CERN has developed a custom radiation resistant version of the easy-to-install =Firefly= optical transceivers [[cite:&versatile_link;&firefly]], optimised for use in the \ac{BE} electronics to communicate with the \ac{VTRx+} modules.
...
To avoid an extremely demanding linearization procedure at \SI{40}{\mega\hertz}, an approximate approach is instead employed, carefully balancing the unavoidable positive and negative errors resulting from the approximations.
...
Only odd-numbered layers are considered for \ac{TC} reduction.
...
The \ac{ECON-T} then builds /module sums/, where the energies of \acp{TC} in a module are summed without any energy threshold being applied.
...
Clusters, together with simpler \acp{TT} built out of module sums along the longitudinal direction, amount to the final \ac{HGCAL} \acp{TP} to be transmitted to \ac{L1}.
...
Additionally, the more data fits into a single \ac{FPGA}, the less data duplication is required to handle boundaries, especially when taking into account that different \ac{BE} \acp{FPGA} do not communicate with eachother.
...
In parallel, module sums are partially summed into module towers, being formed separately for the \ac{CE-E} and \ac{CE-H}.
...
Partial tower energies are accumulated into (\rapidity{}, \ac{phi}) bins and clusters are built following the steps highlighted in [[fig:stage2chain]]:
...
+ *Seeding*:
  ...
  We define the /window size/ to be $k$ based on the number of $k^{\text{th}}\text{-order}$ neighbours considered by the seeding window.
...
+ *Clustering*:
  ...
  Contrary to previous steps, which run on a \coordsa{} space, the clustering uses the $(x/z,\,y/z)$ space.
  ...
  The default matching radius slowly increases with detector's depth, from 0.015 in the first layer to 0.050 in the last \ac{CE-H} layers.
  ...
  The second algorithm, called =max_energy=, prioritizes an association based on seed energy, where the highest energy seed is associated to all \acp{TC} within its matching radius, the second-highest energy seeds is associated to the remaining \acp{TC} within its (different) matching radius, and so forth.
...
#+CAPTION: Illustration in the $(x/z,\,y/z)$ space of the two clustering algorithms considered in the \ac{HGCAL} \ac{TPG}. Black dots represent \acp{TC}, green crosses refer to seeds and the red cross stands for the highest energy seed. The purple circles represent the region of influence of a particular seed on its neighbouring \acp{TC}. (Left) The =min_distance= algorithm associates \acp{TC} based on distance. (Right) The =max_energy= algorithm prioritizes instead association based on the energy of the seeds. Adapted from [[cite:&bruno_chep23]].
#+BEGIN_figure
...
Additionally, two separate \ac{HAD} and \ac{EM} interpretations will be defined, with possibly different parameters, such as radii or energy thresholds.
