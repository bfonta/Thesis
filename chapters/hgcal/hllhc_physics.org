...
A more powerful \ac{LHC} is expected to open up a large physics program, by increasing the accurancy of current measurements and enhancing the sensitivity to rare processes through both direct and indirect searches, with more and more challenging signatures, which occur below the current sensitivity level.
...
\myparagraph{Higgs boson pairs}

\noindent ref:fig:hh_nonres_projections allows the comparison of early HH \run{2} results with current full \run{2} upper cross-section limits.
