...
In order to reproduce the \ac{SR}, the dataset thus obtained is weighted by two sets of weights to account for additional jet activity and subsisting kinematic mismatches.
...
* The problem with \ac{QCD} simulation
...
The transverse thrust axis is in turn defined as the axis where the sum of the absolute values of the $\pt$ projections of the all the jets in the event is maximal.
