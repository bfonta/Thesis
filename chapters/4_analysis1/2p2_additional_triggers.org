...
The $\metnomu$ trigger also enables to reach $\text{high-}|\eta|$ events that would otherwise not be kept by the cross lepton triggers, since no $|\eta|$ cut is enforced.
...
For low resonance masses, the two Higgs bosons lie close to a back-to-back topology in the laboratory frame, and \ac{MET} (and $\metnomu$) largely cancel.
As $\mx$ increases, the bosons become more and more collimated, leading to a \ac{MET} increase driven by the neutrinos from $\tau$ decays.
...
Trigger \acp{SF} are applied to correct for discrepancies between data and \ac{MC} trigger efficiencies, as described in [[#sec:mc_corrections]].
