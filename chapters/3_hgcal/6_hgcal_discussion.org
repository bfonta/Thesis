...
A logical and much awaited extension of this chapter's work consists on using modern tools to include the new seeding algorithm in \acp{FPGA}.
...
\Ac{RTL} can be converted to a bitstream by =Vivado=, which is used as configuration data to program the \acp{FPGA}.
