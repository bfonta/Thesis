...
This has the advantage of reducing the number of free parameters to only \num{3}, usually parameterised as the mixing angle, the ratio of \ac{VEV}'s, and the mass of the new resonance.
...
Exploiting \ac{EW} symmetry breaking, both H and S aquire a \ac{VEV}, where the Higgs \ac{VEV} $v_{0}$ is measured to be \SI{\sim 246}{\GeV}, using Fermi's constant (see [[eq:self_couplings]]), and the new S \ac{VEV} $x_{0}$ is unknown.
