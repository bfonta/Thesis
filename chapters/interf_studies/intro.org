...
The ongoing \run{3} should see the first appearances of non-\ac{NWA} setups, since deviations from the \ac{NWA} approach, including this work, have recently drawn a significant interest from the community.
For simplicity, in this work we consider the resonant model with the smallest number of additional free parameters: the singlet model [[cite:&papa1]].
