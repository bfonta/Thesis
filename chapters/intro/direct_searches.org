...
The analysis represents the first use of the novel Particle Transformer algorithm [[cite:&particle_transformer]], briefly covered in [[#sec:prospects]], to tag a merged $\text{H}\rightarrow\text{V}\text{V} \rightarrow 4\text{q}$ jet.
...
In the same work, interpretations are done for different theories, establishing  exclusion limits on their parameters.
Individual analyses extract 95% \ac{CL} upper limits on the resonant HH production as a function of the resonance mass $\mx$.
When YH limits are drawn, an additional dependence on $\my$ exists.
...
No deviation larger than 2 standard deviations from the expected limits is observed.
...
The \xhhbbtt{} result obtained in this thesis and shown in [[#sec:final_limits]] hints at a much stronger limit than the one in [[fig:res_comb_xhh]], opening a window for further improvements in the combination, still with \run{2} data.
