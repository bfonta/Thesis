...
\ac{CMS} has exploited parking strategies since its inception to explore \ac{NP} in \ac{VBF}, Higgs, \bphys{} or \ac{SUSY} scenarios.
During \run3{} an enhanced B-parking program has been envisaged, with the goal of collecting dimuon and dielectron final states from h hadron decays, in the wake of what has been done in \run{2}.
...
+ *VBF*: The VBF production mode of the Higgs boson drives the sensitivity of some of its decay modes. It also brings an important constraing power to \ac{EFT} coupling measurements, and provides unique acess to \k2v{} in dihiggs processes. VBF trigger offer an alternative to raise thresholds to keep rates under control: the selection of the two forward jets.
...
+ *HH*: As discussed in [[ref:sec:dihiggs]], the study of the Higgs boson self-coupling is one of, if not the most important measurement to be pursued at the \ac{LHC} in the foreseeable future. \ac{CMS} has successfully deployed the \ac{PNet} b-tagging discriminant at \ac{HLT} level, which made possible the loosening of jet $\pt$ thresholds, $\htvar$ selections and b-tagging requirements. The \ac{PNet} addition brought a wonderful signal efficiency improvement of
  https://cds.cern.ch/record/2868787/
  @update above with DPS note from Louis@
