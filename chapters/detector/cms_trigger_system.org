...
The calorimeter trigger takes advantage of the \ac{TMT}, spatially-pipelined mechanism, which enables the treatment of all event data in a single processing board, in contrast with the parallelism that is usually required [[cite:&time_multiplex]].
...
$N$ corresponds to the "depth" of the \ac{TMT}, which equates to the number of available boards with enough memory to process one event in full.
