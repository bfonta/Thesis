...
The electromagnetic calorimeter (ECAL) of the CMS detector provides a highly efficient and accurate reconstruction of photons and electrons over a wide range of energies from low-energy electrons (5 GeV) typ-
ical of multilepton events, to electroweak-scale energies (Higgs and W/Z bosons), up to the
...
\Ac{CMS} is specifically optimized for muon measurements, which is performed by \acp{DT} in the barrel region and \acp{CSC} in the forward region.
...
The barrel section of the muon chambers is composed of four muon stations interleaved with the iron of the yoke, which provide mechanical support.
...
We can indeed find \acp{RPC} present both in the barrel, together with \acp{DT}, and in the endcaps, together with \acp{CSC}.
...
Present in the barrel section, they consist on drift chambers with the goal of providing position resolutions of the order of \SI{100}{\micro\meter}.
...
Stationed in the endcaps, these detectors are multiwire proportional chambers with a segmented cathode readout.

\Acp{CSC} consist of arrays of positively-charged wires perpendicular to negatively-charged \ch{Cu} strips, all within a gas volume.
When muons pass through, they knock electrons off the gas atoms.
Both displaced electrons and ions follow the electric fields, creating signals in the wires and strips, respectively.
...
The charges induced in the cathode strips are used to obtain a precise coordinate along the wires.
Each chamber includes six detecting layers where the strips run radially.
...
They are present in both the barrel and in the endcap.
All muon stations are equipped with at least one \ac{RPC}, but two are present in inner barrel regions to compensate for the lower resolution of low $\pt$ muons.
The low $\pt$ reach of the \ac{CMS} muon trigger is \SI{\sim 4}{\GeV} in the barrel and \SI{\sim 2}{\GeV} in the endcaps.
