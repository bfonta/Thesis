...
Due to the extremely low cross-sections involved, neutrinos are not detected by the \ac{CMS} experiment, or, for that matter, by any of the four large \ac{LHC} experiments.
...
#+NAME: eq:met
\begin{equation}
\ptmiss = - \sum_{i=0}^{\text{N}}\vec{p}_{\text{T,i}}^{\hspace{1mm}} \: ,
\end{equation}

\noindent which are the visible particles interacting via the electromagnetic and strong forces in the final state of the collisions.
