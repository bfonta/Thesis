...
#+CAPTION: Trigger efficiencies as a function of the invariant double Higgs mass for simulated samples. $\kl=1$ is assumed in both plots. Very significant efficiency increases are also observed in the \hhhbbbbbb{} and \hhhbbbbtt{} analyses. (Left) \hhbbbb{}, considering the full \run{2}, 2022 and 2023 data-taking periods. (Right) \hhbbtt{} in 2023 only. Taken from [[cite:&parking_hh_twiki]]; also available in [[cite:&parking_scouting_run3_cms]].
#+BEGIN_figure
