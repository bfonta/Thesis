...
The seed for the next big discovery can nonetheless lie where the SM was completed: the Higgs boson might still hide many surprises, and represent the gateway to a new understanding of fundamental interactions.
Unfortunately, given the remarkable predictive power of the SM, few are expecting a revolution in the near future.
The Higgs boson tri-linear coupling is still left unmeasured, but who does really believe anything else than yet another agreement will be obtained?
Many motivated models introduce new particles addressing the known drawbacks of the SM; but would there be any physicist betting that such resonances will be found in the next two decades?
This Thesis instead proposes a more positive view, where measurements are done not based on hopes but on certainties: the High Energy Physics community has access to some of the most complex pieces of engineering ever built, and measurements should be performed on every conceivable physical quantity, since Nature does not follow human speculation[fn:: "Talk is cheap. Show me the code."].
...
Revolutions are found where one least expects them, and this Thesis pursues such noble goals by providing contributions on detector and analysis work, to be added to the stack of human knowledge which will inevitably lift the veil on Nature's next revelation.
